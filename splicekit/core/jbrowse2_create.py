# splicekit jbrowse2 module
# create config.json pointing to annotation and sequence
# per sample
#    bigwig
#    cram
#    wihtin splicekit/jbrowse2/splicekit_data <-- this is where config.json will be 
# add this config.json to URL of JBrowse2 instance that is served
...
{container} samtools index {cram_fname}
    ...
    for sample in bam_files:
        ...
        if (not (os.path.exists(cram_fname) and os.path.exists(bigwig_fname))) or (force_samples == True):
            ...
            print(f'{module_desc} sample {sample} already processed (.bw and .cram in results/results_jbrowse) --> use "splicekit jbrowse2_create samples -force" to overwrite')
    fsh.close()

def create_jbrowse_config(force_annotation):
    # add reference genome, samples files and genome annotation to config.json and create it first
    ...
    if (not os.path.exists('jbrowse2/splicekit_data/config.json')) or (force_annotation==True):      
        ...
        print('{module_desc} config.json already existing in jbrowse2/splicekit_data --> use "splicekit jbrowse2_create annotation -force" to overwrite')
