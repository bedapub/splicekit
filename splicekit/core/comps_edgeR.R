...
input_fname = paste(input_folder, "/", data_folder, "/", comparison_name, ".tab", sep="")
...
# without intercept
group <- factor(c(rep(c("test"), num_test), rep(c("control"), num_control)), levels = c("control", "test"), labels = c("control", "test"))
design <- model.matrix(~0 + group)
colnames(design) <- c("control", "test") # by default, "group" is added to the design col names, "groupcontrol", "grouptest" in this example
...
output_fname = paste(input_folder, "/results/results_edgeR_", atype, "/", comparison_name, "_difffeature.tab.gz", sep="")
...
output_fname = paste(input_folder, "/results/results_edgeR_", atype, "/", comparison_name, "_altsplice.tab.gz", sep="")
write.table(d, file=gzfile(output_fname), sep="\t", row.names=FALSE, quote=FALSE)
